...
<node TEXT="用户指定存储分区方案" ID="ID_939033777" CREATED="1624354508655" MODIFIED="1624716360482"><richcontent TYPE="NOTE" CONTENT-TYPE="xml/">
<html>
  <head>
...
<node TEXT="网络连接/GPS获取需要长时间" ID="ID_953995090" CREATED="1606977589838" MODIFIED="1624716361668"><richcontent TYPE="NOTE" CONTENT-TYPE="xml/">
<html>
  <head>
...
<node TEXT="上报逻辑需要优化" ID="ID_828374373" CREATED="1624526645485" MODIFIED="1624526717023"><richcontent TYPE="NOTE" CONTENT-TYPE="xml/">
<html>
  <head>
  ...
  <body>
    <p>
      在向服务器上报的时候，当前的逻辑是只有出现了异常的情况才需要上报。
    </p>
    <p>
      
    </p>
    <p>
      实际上当设备的状态恢复正常的时候，也需要上报，告知服务器异常状态在什么时间点已经消除了。
    </p>
  </body>
</html></richcontent>
</node>
...
<html>
  ...
  <body>
    <p>
      使用CoAP协议来作为设备上报状态的协议的时候，需要优化一下网络通信这一块的软件架构。
    </p>
    <p>
      
    </p>
    <p>
      作为设备来说，上报状态的时候，需要发送一条消息，同时一般还是接收一条回复的消息。
    </p>
    <p>
      而且，设备在不用上报状态的时候，也需要监听端口，因为服务器端有可能会主动的发送消息来获取设备的状态信息。
    </p>
    <p>
      
    </p>
    <p>
      所以比较优化的一个软件架构，目前思考来看，是这样的：
    </p>
    <p>
      
    </p>
    <p>
      1，创建一个消息处理线程专门用于处理消息。
    </p>
    <p>
      2，这个线程用于接收所有的进来的coap消息。
    </p>
    <p>
      3，创建一个状态机，或者标置一些FLAG，表明当前系统处于一个什么样的状态，这样能够确定当前的系统应该是接收什么样的消息。
    </p>
    <p>
      4，发送出去的消息，可以不放在这个消息进程中处理，而是直接在那些需要发送消息的地方直接发送。
    </p>
    <p>
...
<html>
  ...
  <body>
    ...
    <p>
      不过使用NB-IoT的结果是，在没有数据交互的时候，设备会关闭Modem的接受功能，进入睡眠状态。在这种情况下，设备虽然不能接受服务器的请求，但此时设备的代码会不会出问题？
    </p>
  </body>
</html></richcontent>
</node>
